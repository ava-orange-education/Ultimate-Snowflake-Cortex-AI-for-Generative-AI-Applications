...
CORTEX_SEARCH_SERVICES = "cortex_ai_db.public.ZENTIME_CUSTOMER_SUPPORT_SERVICE"
SEMANTIC_MODELS = "@cortex_ai_db.public.CORTEX_AI_STG/zentime_sales_support.yaml"
...
def main():
    st.title("ZenTime Sales & Support Assistant")
    ...
    if query := st.chat_input("What question do you have about ZenTime?"):
                ...
                with st.chat_message("assistant"):
                    ...
                    if citations:
                        ...
                        for citation in citations:
                            ...
                            if doc_id:
                                query = f"SELECT chunk from cortex_ai_db.public.docs_chunks_table WHERE category = '{doc_id}'"
                                result = run_snowflake_query(query)
                                result_df = result.to_pandas()
                                if not result_df.empty:
                                    transcript_text = result_df.iloc[0, 0]
                                else:
                                    transcript_text = "No chunks available"
